...
이 프로젝트에서 코드를 변경하거나 기능을 추가/수정할 때 반드시 아래 지침을 따르세요.

### 1. 작업 전 필수 절차

#### 1.1. 계획서 먼저 작성
- **모든 작업 시작 전** Implementation Plan을 작성하고 사용자 승인을 받습니다.
- 계획서에는 **문제 원인**, **수정 방안**, **영향 범위**, **검증 계획**을 포함합니다.

#### 1.2. 영향 범위 파악
- 수정하려는 파일이 어떤 서비스에 속하는지 확인합니다:
  - `services/ocr.ts` 수정 → OCR 파이프라인 전체 테스트 필요
  - `services/ai/OpenAIService.ts` 수정 → 모든 ScanType 케이스 확인
  - `services/supabase-modules/` 수정 → RLS 정책 영향 확인
  - `app/_layout.tsx` 수정 → 앱 전체 초기화/인증 흐름 검증
  - `constants/categories.ts` 수정 → 분류 로직 전체에 파급 효과
